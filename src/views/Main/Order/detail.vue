  ...
  <div>
    <n-spin :show="loading">
      ...
      <n-layout class="upload" style="display: relative" :native-scrollbar="false">
        <n-layout-header class="upload-header">
          <div class="left">
            ...
            <span :class="{ active: active && data.orderStatus >= 5, disabled: data.orderStatus < 5 }" style="margin-left: 20px" @click="data.orderStatus >= 5 ? getData(1) : null"
              >精修图</span
            >
          </div>
          <div class="right">
            ...
            <input type="file" ref="touchfileInput" style="display: none" @change="handleTouchFileChange" multiple />
            ...
            <n-popconfirm @positive-click="handleReverse">
              <template #trigger>
                <template v-if="data.orderStatus === 6 && active">
                  <n-button style="margin-right: 20px"> 重新提交精修图 </n-button>
                </template>
                <template v-else>
                  <span></span>
                </template>
              ...
              <div class="image-group-touch-content">
                <n-layout-content class="image-group-touch-content-wrap" :native-scrollbar="false">
                  <div class="image-group-touch-content-1">
                    <ImageDisplay v-for="(item, index) in picData" :key="index" :pic-data="item" :index="index" :pic-list="picData" />
                  </div>
                </n-layout-content>
                <n-layout-content
                  class="image-group-touch-content-wrap"
                  :native-scrollbar="false"
                  @dragover.prevent="isDragging = true"
                  @dragleave.prevent="isDragging = false"
      ...
      <n-modal v-model:show="uploadModal" :mask-closable="false">
        <n-card style="width: 600px" title="正在上传中请勿关闭此窗口" :bordered="false" size="small" role="dialog" aria-modal="true">
          <template #header-extra>
            <n-button @click="handleUploadAbort" :bordered="false">停止</n-button>
          </template>
          <div class="uploadModal-layout"> 上传进度 {{ uploadNumberSuccess + '/' + uploadNumberTotal }} </div>
        </n-card>
      </n-modal>
    </n-spin>
  </div>
...
<script setup lang="ts">
  ...
  const uploadStatusText = computed(() => {
    if (active.value) {
      if (data.value.orderStatus === 5) {
        return `顾客选图 ${picDataTotal.value} 张 / 已上传精修图 ${localTouchPictures.value.length} 张`;
      }
      return `顾客选图 ${picDataTotal.value} 张 / 已精修 ${touchDataTotal.value} 张`;
    }
    const totalPics = localPictures.value.length || picData.value.length;
    const requiredPics = 12;
    return `已上传 ${totalPics} 张底图 (至少需要${requiredPics}张)`;
  });
  ...
  const handleSubmitOriginal = async () => {
    try {
      if (localPictures.value.length < 12) {
        message.error('底图数量不足，需要至少12张底图');
        return;
      }
      uploadModal.value = true;
      uploadNumberSuccess.value = 0;
